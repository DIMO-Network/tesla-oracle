...
func InitializeServices(ctx context.Context, logger *zerolog.Logger, settings *config.Settings) (*Services, error) {
	...
	riverClient, dbPool, err := initializeRiver(ctx, *logger, settings, identityService, &pdb, transactionsClient, walletService, teslaFleetAPIService, tokenManager, repositories, teslaService)
	if err != nil {
...
func initializeRiver(ctx context.Context, logger zerolog.Logger, settings *config.Settings, identityService service.IdentityAPIService, dbs *db.Store, tr *transactions.Client, ws service.SDWalletsAPI, teslaFleetAPI core.TeslaFleetAPIService, tokenManager *core.TeslaTokenManager, repositories *repository.Repositories, teslaService *service.TeslaService) (*river.Client[pgx.Tx], *pgxpool.Pool, error) {
	...
	teslaCommandWorker := work.NewTeslaCommandWorker(teslaFleetAPI, tokenManager, teslaService, repositories.Command, repositories.Vehicle, &logger, 1*time.Minute, settings.MobileAppDevLicense)
