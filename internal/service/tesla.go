...
func (ts *TeslaService) ProcessAuthCodeExchange(ctx context.Context, authCode, redirectURI string, requiredScopes []string) (*core.TeslaAuthCodeResponse, error) {
	...
	if err != nil {
		...
		// Wrap any other unexpected errors (preserve full error chain with %w: %w for Go 1.20+)
		return nil, fmt.Errorf("%w: %w", core.ErrCredentialStore, err)
	}
...
func (ts *TeslaService) GetVehicleStatus(ctx context.Context, tokenID int64, requiredScopes []string) (*models.StatusDecision, error) {
	// Get synthetic device
	sd, err := ts.repositories.Vehicle.GetSyntheticDeviceByTokenID(ctx, tokenID)
	if err != nil {
...
func (ts *TeslaService) validateAccessTokenWithScopes(accessToken string, requiredScopes []string) error {
	...
	if err != nil {
		return fmt.Errorf("%w: %w", core.ErrInvalidAccessToken, err)
	}
